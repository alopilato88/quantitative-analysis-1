...
The distribution of a given variable gives the frequency of each value of the variable. This frequency can be in either absolute terms (count of observations) or relative terms (proportion or percent). A variable's distribution completely describes the variable.

## Importance of Plots

Plotting your data immediately gives you more information than looking at the raw numbers: 

* Plots give you visual information about the center, spread, and shape of your data.
* Plots alert you to values that are really far away from the center of your data.
...
A statistic is a number that we can calculate from our data. Statistics provide useful information about the characteristics of a variable's distribution such as its center (mean) or spread (standard deviation).  
...
**Population parameters** (or parameters) are numerical summaries of our population. **Statistics** are estimates of these parameters calculated from data sampled from this population. 
...
* $\mu_{\text{AI Low Exp.}} \space - \space \mu_{\text{AI Med. Exp.}} = 0$
* $\mu_{\text{AI Low Exp.}} \space - \space \mu_{\text{AI Hifg Exp.}} = 0$
